...
fn test_default_device_change_triggers_callback_multiple(scope_types: &[(Scope, DeviceType)]) {
    ...
    struct CallbackData {
        counts: Arc<(Mutex<CallbackCounts>, Condvar)>,
    }
    ...
    let counts = Arc::new((Mutex::new(CallbackCounts::default()), Condvar::new()));
    ...
    extern "C" fn input_device_collection_changed_callback(
        ...
        let data = unsafe { &*(user_ptr as *const CallbackData) };

        let (lock, cvar) = &*data.counts;
    ...
    for (scope, device_type) in scope_types {
        ...
        let output = Command::new("SwitchAudioSource")
            .args(&["-t", scope_name, "-i", &alternate_device.to_string()])
            .output()
            .expect("Failed to execute SwitchAudioSource");
        assert!(output.status.success());

        // Wait for callback
        let expected_count_before = match *device_type {
            DeviceType::INPUT => result_guard.input_count,
            DeviceType::OUTPUT => result_guard.output_count,
    ...
    for (scope, _) in scope_types {
        ...
        Command::new("SwitchAudioSource")
            .args(&["-t", scope_name, "-i", &initial_default.to_string()])
            .output()
