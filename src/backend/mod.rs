...
fn audiounit_mix_output_buffer(
    stm: &mut AudioUnitStream,
    output_frames: usize,
    ...
    assert!(
        input_buffer_size
            >= cubeb_sample_size(stm.output_stream_params.format())
                * stm.output_stream_params.channels() as usize
                * output_frames
    );
...
fn minimum_resampling_input_frames(stm: &AudioUnitStream, output_frames: i64) -> i64 {
    ...
    (stm.input_hw_rate * output_frames as f64 / f64::from(stm.output_stream_params.rate())).ceil()
        as i64
}
...
fn audiounit_reinit_stream(stm: &mut AudioUnitStream, flags: device_flags) -> Result<()> {
    ...
    assert!(
        (stm.input_unit.is_null() || flags.contains(device_flags::DEV_INPUT))
            && (stm.output_unit.is_null() || flags.contains(device_flags::DEV_OUTPUT))
    );
    if !*stm.shutdown.get_mut() {
...

    {
        ...
        if flags.contains(device_flags::DEV_INPUT)
            && audiounit_set_device_info(stm, input_device, io_side::INPUT).is_err()
        {
...
fn audiounit_layout_init(stm: &mut AudioUnitStream, side: io_side) {
    ...
    stm.context.layout.store(
        audiounit_get_current_channel_layout(stm.output_unit),
        atomic::Ordering::SeqCst,
    );
    let _ = audiounit_set_channel_layout(
        stm.output_unit,
        io_side::OUTPUT,
        stm.context.layout.load(atomic::Ordering::SeqCst),
    );
}
...
fn audiounit_create_aggregate_device(stm: &mut AudioUnitStream) -> Result<()> {
    if let Err(r) =
        audiounit_create_blank_aggregate_device(&mut stm.plugin_id, &mut stm.aggregate_device_id)
    {
        cubeb_log!(
...
fn audiounit_configure_input(stm: &mut AudioUnitStream) -> Result<()> {
    ...
    cubeb_log!(
        ...
        stm.input_stream_params.format(),
        stm.latency_frames
    );
...
fn audiounit_configure_output(stm: &mut AudioUnitStream) -> Result<()> {
        ...
        || stm.context.layout.load(atomic::Ordering::SeqCst) != stm.output_stream_params.layout()
    {
        ...
        stm.output_desc.mBytesPerFrame =
            (stm.output_desc.mBitsPerChannel / 8) * stm.output_desc.mChannelsPerFrame;
...
fn audiounit_setup_stream(stm: &mut AudioUnitStream) -> Result<()> {
    ...
    if has_input(stm) && has_output(stm) && stm.input_device.id != stm.output_device.id {
    ...
    stm.resampler.reset(unsafe {
        ffi::cubeb_resampler_create(
            ...
            if stm_has_output {
                stm.output_stream_params.as_ptr()
            } else {
                ptr::null_mut()
            },
            target_sample_rate,
    ...
    if !stm.output_unit.is_null() {
        ...
        {
            stm.current_latency_frames.fetch_add(
                (unit_s * stm.output_desc.mSampleRate) as u32,
                atomic::Ordering::SeqCst,
            );
        }
    }
    ...
    if !stm.input_unit.is_null() && !stm.output_unit.is_null() {
        ...
        stm.expected_output_callbacks_in_a_row =
            (stm.output_hw_rate / stm.input_hw_rate).ceil() as i32
    }
...
fn audiounit_add_device_listener(
    context: &mut AudioUnitContext,
    devtype: DeviceType,
    ...
    assert!(
        devtype.contains(DeviceType::INPUT) && context.input_collection_changed_callback.is_none()
...
fn audiounit_remove_device_listener(
    context: &mut AudioUnitContext,
    devtype: DeviceType,
) -> OSStatus {
    context.mutex.assert_current_thread_owns();
...
impl ContextOps for AudioUnitContext {
    ...
    fn stream_init(
        ...
        if let Some(stream_params_ref) = input_stream_params {
            ...
            if let Err(r) = audiounit_set_device_info(
                boxed_stream.as_mut(),
                input_device as AudioDeviceID,
                io_side::INPUT,
            ) {
                cubeb_log!(
        ...
        if let Some(stream_params_ref) = output_stream_params {
            ...
            if let Err(r) = audiounit_set_device_info(
                boxed_stream.as_mut(),
                output_device as AudioDeviceID,
                io_side::OUTPUT,
            ) {
                cubeb_log!(
    ...
    fn register_device_collection_changed(
        ...
        if collection_changed_callback.is_some() {
            ret =
                audiounit_add_device_listener(self, devtype, collection_changed_callback, user_ptr);
        } else {
